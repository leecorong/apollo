...
import com.ctrip.framework.apollo.portal.spi.letv.LeLogoutHandler;
import com.ctrip.framework.apollo.portal.spi.letv.LeUserService;
import com.ctrip.framework.apollo.portal.spi.letv.LeUserinfoHolder;
import com.ctrip.framework.apollo.portal.spi.letv.configuration.LeContextConfiguration;
...
public class AuthConfiguration {
    ...
    @Configuration
    @Profile("letv")
    static class LetvAuthAutoConfiguration {

        @Bean
        @ConditionalOnMissingBean(UserInfoHolder.class)
        public LeUserinfoHolder LeUserInfoHolder() {
            return new LeUserinfoHolder();
        }

        @Bean
        @ConditionalOnMissingBean(LogoutHandler.class)
        public LeLogoutHandler logoutHandler() {
            return new LeLogoutHandler();
        }

        @Bean
        @ConditionalOnMissingBean(UserService.class)
        public LeUserService defaultUserService() {
            return new LeUserService();
        }
    ...
    static class SpringSecurityAuthAutoConfiguration {
        ...
        public JdbcUserDetailsManager jdbcUserDetailsManager(AuthenticationManagerBuilder auth, DataSource datasource) throws Exception {
            JdbcUserDetailsManager jdbcUserDetailsManager = auth.jdbcAuthentication().passwordEncoder(new BCryptPasswordEncoder())
                    .dataSource(datasource).usersByUsernameQuery("select Username,Password,Enabled from `Users` where Username = ?")
                    .authoritiesByUsernameQuery("select Username,Authority from `Authorities` where Username = ?").getUserDetailsService();
    ...
    static class SpringSecurityLDAPConfigurer extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(AuthenticationManagerBuilder auth) throws Exception {
            auth.ldapAuthentication().userDnPatterns(ldapProperties.getUserDnPatterns()).contextSource(ldapContextSource).passwordCompare()
     ...
     */
    @Configuration
    @ConditionalOnMissingProfile({ "ctrip", "auth", "ldap", "letv" })
